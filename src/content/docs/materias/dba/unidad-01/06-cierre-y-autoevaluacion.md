...
description: "Unidad 1 de DBA. Cierre integrador, autoevaluación de 11 preguntas, glosario y qué sigue en la materia."
---
...
Con lo que trabajaste en esta unidad, ya puedes explicarla con precisión.
Faltaba alguien con la responsabilidad explícita del rol de DBA. Esa
falla es justo el tipo de problema que las responsabilidades operativas
cotidianas (monitoreo, diagnóstico, documentación) deberían haber
cubierto. Faltaba una separación real de ambientes que impidiera
modificar producción directamente. Y quien lo hiciera necesitaba
comprender, al menos a nivel conceptual, la arquitectura de la base de
datos que administraba, relacional o NoSQL.
...
| Organización física (almacenamiento) | Concepto universal: el archivo o archivos reales en disco donde viven los datos, sin importar el motor. Ejemplo en Oracle: datafile. En PostgreSQL: archivos dentro de `PGDATA`. |
| Tablespace | Unidad de almacenamiento lógico de una base de datos relacional que agrupa estructuras relacionadas (terminología de Oracle. En PostgreSQL también existe el concepto, con un alcance distinto). |
...
Más adelante en la materia vas a aplicar controles de seguridad y acceso sobre un
...
se dan por conocidos: las unidades siguientes no los vuelven a explicar desde cero.
...
Este material se apoya en las referencias verificadas de la unidad,
