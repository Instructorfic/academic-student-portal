...
> **Nota.** Esta sección presenta cada concepto primero de forma
> independiente de cualquier motor, y luego usa Oracle y PostgreSQL como
> **ejemplos ilustrativos** de cómo se ve ese concepto en un producto
> real. Es el mismo enfoque que se usa en
> [Arquitectura NoSQL](/materias/dba/unidad-01/05-arquitectura-nosql/)
> para los modelos NoSQL.
...
antes de que produzcas tu propio diagrama en la Actividad 5. Ese diagrama
sí debe etiquetar la organización lógica, la física y la memoria de la
instancia, usando de forma consistente la terminología de uno de los dos
motores (Oracle: tablespace, datafile, SGA, Database Buffer Cache /
PostgreSQL: PGDATA, archivos de datos, `shared_buffers`) y explicando la
